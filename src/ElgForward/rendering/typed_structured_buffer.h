...
template<typename T>
inline TypedHandle<T> Create(size_t name_hash, size_t max_size, T* initial_data, size_t initial_count, ID3D11Device* device) {
...
template<typename T>
inline TypedHandle<T> Create(const std::string& name, size_t max_size, T* initial_data, size_t initial_count, ID3D11Device* device) {
...
template<typename T>
inline T* GetElementAt(TypedHandle<T> handle, size_t index) {
...
template<typename T>
inline Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> GetShaderResourceView(TypedHandle<T> handle) {
  return GetShaderResourceView(static_cast<Handle>(handle));
}

template<typename T>
inline void SetCurrentSize(TypedHandle<T> handle, size_t new_size) {
  SetCurrentSize(static_cast<Handle>(handle), new_size);
}

template<typename T>
inline size_t GetCurrentSize(TypedHandle<T> handle) {
  return GetCurrentSize(static_cast<Handle>(handle));
}

template<typename T>
inline bool Add(TypedHandle<T> handle, T* value) {
  return Add(static_cast<Handle>(handle), value);
}

template<typename T>
inline size_t GetMaxSize(TypedHandle<T> handle) {
