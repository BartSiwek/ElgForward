...
struct DirectionalLight {
  ...
  DirectionalLight(float world_x, float world_y, float world_z,
    ...
    DirectionWorldSpace = DirectX::XMVectorSet(world_x, world_y, world_z, 0.0f);
  ...
  void* operator new(size_t size) {
    return aligned_new<DirectionalLight>(size);
  }
  ...
  void* operator new[](size_t size) {
    return aligned_new_array<DirectionalLight>(size);
  }
